...
def main_app():
    ...
    # Top bar with title and logout on right
    col_left, col_right = st.columns([9, 1])
    with col_left:
        st.image("https://images.unsplash.com/photo-1529070538774-1843cb3265df?auto=format&fit=crop&w=400&q=80",
                 width=80, caption=None)
        st.markdown("<h1 style='color:#114b5f; display:inline-block; padding-left:10px;'>Student Academic & Financial Records Dashboard</h1>", unsafe_allow_html=True)
    with col_right:
        # Use st.button with an emoji or unicode icon to simulate profile icon
    ...
    if uploaded_file:
        try:
            ...
            required = ["Stu_Internal_marks", "Stu_external_Marks",
                        "Stu_total_Classes", "Stu_attended",
                        "Stu_tution_Fees", "Stu_fee_paid"]
